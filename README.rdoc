...
Ideally I suppose it would work with something like EventMachine, but I havent
worked enough with event machine to know if that's the case. Its meant for cases
where blocks are passed to methods, not as is usual, as a lightweight strategy
pattern, but to sequence a set of steps. Like in this example.
...
While each function takes a block, this is merely to keep the rest of the program.
...
So step rewrites this code into the code above and evaluates it.
...
If step is a method that exists in your code, you can instead use the module method
...
if _ is a variable you are already using, you can configure the callback for
rewriting.
...
The problem with the current approach is that it does not cache the result of rewriting
